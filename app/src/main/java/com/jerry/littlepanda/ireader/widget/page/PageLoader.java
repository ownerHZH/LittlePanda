...
import com.jerry.littlepanda.ireader.model.bean.BookRecordBean;
...
import com.jerry.littlepanda.ireader.utils.ScreenUtils;
...
public abstract class PageLoader{
    ...
    private void initData(){
        ...
        mParagraphSize = ScreenUtils.dpToPx(DEFAULT_PARAGRAPH_INTERVAL);
    }
    ...
    public void setBgColor(int theme){
        if (isNightMode && theme == ReadSettingManager.NIGHT_MODE){
            mTextColor = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_font_night);
            mPageBg = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_bg_night);
        }
        else if (isNightMode){
            mBgTheme = theme;
            ...
            switch (theme){
                case ReadSettingManager.READ_BG_DEFAULT:
                    mTextColor = ContextCompat.getColor(APPAplication.getContext(),R.color.nb_read_font_1);
                    mPageBg = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_bg_1);
                    break;
                case ReadSettingManager.READ_BG_1:
                    mTextColor = ContextCompat.getColor(APPAplication.getContext(),R.color.nb_read_font_2);
                    mPageBg = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_bg_2);
                    break;
                case ReadSettingManager.READ_BG_2:
                    mTextColor = ContextCompat.getColor(APPAplication.getContext(),R.color.nb_read_font_3);
                    mPageBg = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_bg_3);
                    break;
                case ReadSettingManager.READ_BG_3:
                    mTextColor = ContextCompat.getColor(APPAplication.getContext(),R.color.nb_read_font_4);
                    mPageBg = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_bg_4);
                    break;
                case ReadSettingManager.READ_BG_4:
                    mTextColor = ContextCompat.getColor(APPAplication.getContext(),R.color.nb_read_font_5);
                    mPageBg = ContextCompat.getColor(APPAplication.getContext(), R.color.nb_read_bg_5);
                    break;
            }
        }
    ...
    List<TxtPage> loadPages(TxtChapter chapter, BufferedReader br){
        ...
        //剩余高度
        int rHeight = mVisibleHeight + mIntervalSize + mParagraphSize;
        ...
        try {
            while ((paragraph = br.readLine()) != null){
                ...
                while (paragraph.length() > 0){
                    //重置剩余距离
                    rHeight -= (mTextPaint.getTextSize() + mIntervalSize);
