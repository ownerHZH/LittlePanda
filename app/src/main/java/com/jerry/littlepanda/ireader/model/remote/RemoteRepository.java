...
public class RemoteRepository {
    ...
    public Single<List<BookChapterBean>> getBookChapters(String bookId){
        return mBookApi.getBookChapterPackage(bookId, "chapter").map(new Function<BookChapterPackage, List<BookChapterBean>>() {

            @Override
            public List<BookChapterBean> apply(@NonNull BookChapterPackage bookChapterPackage) throws Exception {
                if(null!=bookChapterPackage.getMixToc())
                {
                    return bookChapterPackage.getMixToc().getChapters();
                }
                return new ArrayList<BookChapterBean>();
            }
        });
