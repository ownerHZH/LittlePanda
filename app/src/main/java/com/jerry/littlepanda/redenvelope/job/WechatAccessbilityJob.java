...
import com.jerry.littlepanda.redenvelope.IStatusBarNotification;
import com.jerry.littlepanda.redenvelope.QiangHongBaoService;
import com.jerry.littlepanda.redenvelope.util.AccessibilityHelper;
import com.jerry.littlepanda.redenvelope.util.NotifyHelper;
...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(event == Config.WX_AFTER_OPEN_HONGBAO) { //拆红包
            if (wechatVersion < USE_ID_MIN_VERSION) {
                ...
                if(targetNode==null)
                {
                    targetNode = AccessibilityHelper.findNodeInfosByText(nodeInfo, "開");
                }
        ...
        if(targetNode != null) {
            ...
            if(event == Config.WX_AFTER_OPEN_HONGBAO) {
                APPAplication.eventStatistics(getContext(), "open_hongbao");
            } else {
                APPAplication.eventStatistics(getContext(), "open_see");
            }
        }
    }
    ...
    private void handleChatListHongBao() {
        ...
        if(list != null && list.isEmpty()) {
            ...
            if(node==null)
            {
                node = AccessibilityHelper.findNodeInfosByText(nodeInfo, "微信红包");
            }
            if(node != null) {
                /*
